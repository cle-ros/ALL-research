...
def q_learning_estimator(dim, action, reward, value_old, learning_rate=.01, flatten=False):
    """
    This function computes the reward/value estimation from the current reward and the previous estimate.
    ...
    """
    if not flatten:
        action_vector = np.zeros(dim)
        action_vector[action] = 1.
    else:
        action_vector = 1.
    reward_action_vector = action_vector * reward
    value = value_old + learning_rate * (reward_action_vector - np.multiply(action_vector, value_old))
    # dprint(value, value_old, reward, learning_rate, reward_action_vector, action_vector)
    return value


def decaying_average_estimator(value_histo, decaying_rate=.9, last_value=None):
    """
