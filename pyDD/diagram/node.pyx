...
class Node(object):
    ...
    def plot(self, name):
        ...
        types = ['solid', 'dashed', 'dotted', 'bold']
        seen = []
        for node in self.nodes:
            if not node in seen:
                if node.is_leaf():
                    ...
                    for child in node.child_nodes:
                        graph.add_node(pd.Node(str(node.__hash__()), label='v'+str(node.d)))
                        if child in node.offsets:
                            try:
                                no = str(node.offsets[child][0]) + '/' + str(node.offsets[child][1])
                            except IndexError:
                                no = str(node.offsets[child])
                        else:
                            no = '-'
                        graph.add_edge(pd.Edge(str(node.__hash__()), str(node.child_nodes[child].__hash__()),
                                               label=no, style=types[child]))
                        seen.append(node)
