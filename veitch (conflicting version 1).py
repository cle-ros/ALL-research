...
def markEdgesRecursively(tree,node):
    if all children edges == 0 or there are no child edges:
        edge to parent (node) = 0
        markEdgesRecursively(parent)
    return
...
def sortNodes(matrix, nullValue):
    # for the time being, this function uses a very inefficient approach
    # labelling all edges that end in the nullValue recursively
    for node in bTree.nodes_iter:
        if node is nullValue:
            edge to parent --> 0
